...
const QuizCard = ( {quiz} : Props) => {
  ...
  return (
    ...
    <div className='bg-[#3D516D] hover:bg-[#4F6B92] cursor-pointer rounded-[10px] h-[250px] relative flex flex-col'>
        ...
        <header className='flex justify-center pt-[25px] font-sora font-semibold text-[20px] text-white'>{quiz.title}</header>
        <div className='flex justify-center p-[50px]'>
            <div className='bg-[#557199] w-fit px-3 py-2 rounded-[30px]'><p className='text-white text-[14px] font-sora'>{quiz.terms} terms</p></div> 
        </div>
    </div>
    </Link>
